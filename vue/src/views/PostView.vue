    ...
    <div style="border-radius: 10px;overflow: hidden;background-color: #FFFFFF">
      ...
      <div class="information-div">
        <div class="like-div">
          <div style="display: inline-block">
            <div style="display: flex;flex-direction: row;text-align: center;gap: 10px">
              <waku-button color="#FFFFFF" hover-color="rgba(0,0,0,0.4)" :font-color="info.post.self_vote ? '#FF4A80' : '#000000'" style="width: 40px" :enable="true" @click="onLikeClick">
              ...
              <waku-button color="#FFFFFF" hover-color="rgba(0,0,0,0.4)" :font-color="info.post.self_fav ? '#FF4A80' : '#000000'" style="width: 40px" :enable="true" @click="onFavoriteClick">
        ...
        <div style="display: flex;flex-direction: row;margin: 20px 0;gap: 20px">
          ...
          <waku-input style="background-color: rgba(0, 0, 0, 0.02);width: 100%" label="发表评论" v-model="info.inputValue">
          ...
          <waku-button style="width: 100px;text-align: center" :enable="true" @click="onSend">发送</waku-button>
        </div>
...
import { getImageByID, goto, comment, postDetail, tag, getComments, image, createComment, vote, favorite } from "@/assets/js/api";
...
const onLikeClick = () => {
  console.log('like')
  info.post.self_vote = info.post.self_vote === 1 ? 0 : 1
  vote(info.post.post_id, info.post.self_vote ? 'up' : 'cancel')
}

const onFavoriteClick = () => {
  console.log('favorite')
  info.post.self_fav = !info.post.self_fav
  favorite(info.post.post_id, info.post.self_fav)
}
...
const onSend = async () => {
  ...
  await createComment(props.id, info.currentReply ? info.currentReply.comment_id as string : '0', info.inputValue);
  info.inputValue = ''
  info.comments = await getComments(props.id, 1)
}
</script>
